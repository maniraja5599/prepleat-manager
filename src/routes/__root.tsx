...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      { title: "Eyas Saree Drapist — PrePleat & Drape" },
      { name: "description", content: "Mobile-first manager for PrePleat & Drape bookings, payments, and customer reminders." },
      { property: "og:title", content: "Eyas Saree Drapist — PrePleat & Drape" },
      { name: "twitter:title", content: "Eyas Saree Drapist — PrePleat & Drape" },
...
function RootComponent() {
  ...
  return (
    <QueryClientProvider client={queryClient}>
      <ThemeApplier />
      <Outlet />
      <Toaster position="bottom-center" duration={2200} closeButton={false} richColors={false} toastOptions={{ style: { background: "var(--color-card)", color: "var(--color-foreground)", border: "1px solid var(--color-border)", fontSize: "13px" } }} />
    </QueryClientProvider>
  );
}
