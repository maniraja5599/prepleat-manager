...
import { IndianRupee, Plus, X, Upload, Minus, LogOut, Cloud, Download, RotateCcw, Palette, Database, User, Trash2, RotateCw, Activity, Undo2, Redo2, Tag, AlertTriangle, Sparkles, HelpCircle, Search, CreditCard, Lock, Unlock } from "lucide-react";
...
const THEMES: { id: ThemeName; label: string; bg: string; fg: string; card: string; primary: string; accent: string; border: string }[] = [
  ...
  { id: "sunset",   label: "Sunset Coral", bg: "#fef7f2", fg: "#3e1b10", card: "#ffffff", primary: "#c85a3a", accent: "#f4cca8", border: "#ebdcd0" },
  { id: "ocean",    label: "Ocean Breeze", bg: "#f2f8fc", fg: "#102638", card: "#ffffff", primary: "#2c78a0", accent: "#a8daf4", border: "#d0e2eb" },
  { id: "forest",   label: "Forest Moss",  bg: "#f5f8f5", fg: "#1a2e1c", card: "#ffffff", primary: "#3c784e", accent: "#c8ecd0", border: "#d2e4d6" },
  { id: "vintage",  label: "Vintage Plum", bg: "#fbf6fc", fg: "#301438", card: "#ffffff", primary: "#8c3c90", accent: "#eccef0", border: "#e4d2e8" },
];
      ...
      {tab === "pricing" && (
        <>
          <Section title="Pricing per saree">
            <p className="text-[11px] text-muted-foreground mb-4">Direct client and artist rates. Tap +/- to step by ₹50.</p>
            <div className="space-y-5">
              {/* PrePleat Section */}
              <div>
                <h4 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2">PrePleat Rates</h4>
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1.5 w-full">
                    <label className="text-[10px] text-muted-foreground uppercase font-semibold">Client Rate</label>
                    <PriceStepper value={settings.prepleatPrice} onChange={(v) => update({ prepleatPrice: v })} />
                  </div>
                  <div className="flex flex-col gap-1.5 w-full">
                    <label className="text-[10px] text-muted-foreground uppercase font-semibold">Artist Rate</label>
              ...
              <div className="border-t border-border/40 pt-4">
                <h4 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2">Drape Rates</h4>
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1.5 w-full">
                    <label className="text-[10px] text-muted-foreground uppercase font-semibold">Client Rate</label>
      ...
      {tab === "theme" && (
        <>
          ...
          <Section title="Color Palettes">
            <p className="text-xs text-muted-foreground mb-3">Tap a preset palette to apply instantly.</p>
              ...
              {THEMES.map((t) => {
                ...
                return (
                  ...
                  >
                    ...
                    {active && <p className="text-[10px] opacity-70 uppercase tracking-wider mt-0.5 font-bold">Active</p>}
                  </button>
                );
              })}
            </div>
            ...
            <div className="p-4 rounded-2xl border-2 border-dashed border-border bg-card/30">
              <div className="flex items-center justify-between mb-2.5">
                ...
                {settings.theme === "custom" && (
                  <span className="text-[10px] uppercase tracking-wider text-primary font-bold bg-primary/10 px-2.5 py-0.5 rounded-full">Active Custom Theme</span>
                )}
              </div>
              <p className="text-[11px] text-muted-foreground mb-4">Pick a quick starting color combo, or customize each color below.</p>

              {/* Curated mood presets */}
              <div className="mb-4">
                <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold mb-2">Curated Mood Presets</p>
                <div className="flex flex-wrap gap-2">
                  {[
                    { label: "Teal Mint", primary: "#0f766e", accent: "#2dd4bf", background: "#f0fdfa", card: "#ffffff", foreground: "#115e59" },
                    { label: "Indigo", primary: "#4338ca", accent: "#a5b4fc", background: "#eef2ff", card: "#ffffff", foreground: "#312e81" },
                    { label: "Coral Rose", primary: "#e11d48", accent: "#fda4af", background: "#fff1f2", card: "#ffffff", foreground: "#881337" },
                    { label: "Ocean Sky", primary: "#0369a1", accent: "#38bdf8", background: "#f0f9ff", card: "#ffffff", foreground: "#0c4a6e" },
                    { label: "Sage Forest", primary: "#15803d", accent: "#4ade80", background: "#f0fdf4", card: "#ffffff", foreground: "#14532d" },
                  ].map((p) => (
                    <button
                      key={p.label}
                      type="button"
                      onClick={() => {
                        update({
                          theme: "custom",
                          customColors: { primary: p.primary, accent: p.accent, background: p.background, card: p.card, foreground: p.foreground },
                    ...
                    >
                      <span className="size-2.5 rounded-full" style={{ background: p.primary }} />
                      {p.label}
                    </button>
                  ))}
                </div>
              ...
              {(() => {
                ...
                const customColorsState = {
                  primary: settings.customColors?.primary || settings.customPrimary || activeThemeObj.primary,
                ...
                return (
                  <>
                    <div className="mb-4 p-3.5 rounded-xl border border-border bg-secondary/30">
                      <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest block mb-2">Live Theme Preview</span>
                      <div className="rounded-xl p-3 border border-border shadow-sm transition duration-200" style={{ background: customColorsState.background, color: customColorsState.foreground }}>
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-1.5">
                            <span className="size-3.5 rounded-full" style={{ background: customColorsState.primary }} />
                            <span className="text-xs font-bold font-display">Business Header</span>
                          </div>
                          <span className="text-[9px] px-2 py-0.5 rounded-full font-semibold" style={{ background: customColorsState.accent, color: customColorsState.foreground }}>Active Tab</span>
                        </div>
                        <div className="rounded-lg p-2.5 border border-border/80 shadow-xs" style={{ background: customColorsState.card, color: customColorsState.foreground }}>
                          <p className="text-xs font-bold">Mock Saree Booking Card</p>
                          <p className="text-[9px] opacity-75">Delivery: Mon, Jun 15 · 10:00 AM</p>
                          <div className="flex justify-between items-center mt-2 pt-1 border-t border-border/30">
                            <span className="text-[10px] font-semibold" style={{ color: customColorsState.primary }}>₹1,200 due</span>
                            <button type="button" className="px-2 py-1 rounded-full text-[9px] font-bold uppercase tracking-wider text-white border-0" style={{ background: customColorsState.primary }}>
                              Action Button
                            </button>
                          </div>
                        </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-5 gap-3.5 mb-4">
                      {([
                        { key: "primary" as const, label: "Brand Primary", desc: "Buttons & highlights" },
                        { key: "accent" as const, label: "Accent Tone", desc: "Pills & borders" },
                        { key: "background" as const, label: "App Backdrop", desc: "Page background" },
                        { key: "card" as const, label: "Cards & Sheets", desc: "Component background" },
                        { key: "foreground" as const, label: "Text & Icons", desc: "Main copy color" },
                      ]).map((c) => {
                        const currentVal = customColorsState[c.key];
                        return (
                          <div key={c.key} className="flex flex-col justify-between gap-1.5 p-2.5 rounded-xl bg-card border border-border shadow-xs hover:border-primary/50 transition">
                            <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider leading-tight">{c.label}</span>
                            <div className="flex items-center gap-1.5 my-1.5">
                              <div className="relative size-8 rounded-full border border-border/80 overflow-hidden shadow-xs cursor-pointer">
                                ...
                                />
                                <div className="absolute inset-0 rounded-full border" style={{ backgroundColor: currentVal }} />
                              </div>
                              <span className="text-[9px] font-mono uppercase text-muted-foreground">{currentVal}</span>
                            </div>
                            <span className="text-[9px] text-muted-foreground/85 leading-tight">{c.desc}</span>
                          </div>
                        );
                      })}
                    </div>
          ...
          <Section title="Calendar Dot Colors">
            <p className="text-xs text-muted-foreground mb-4">Customize the color of the small booking dots shown on the calendar cells.</p>

            {/* Interactive Day Cell Preview */}
            <div className="mb-4 p-3.5 rounded-xl border border-border bg-secondary/30">
              <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest block mb-2">Day Dot Preview</span>
              <div className="flex justify-center">
                <div className="w-14 h-14 rounded-2xl bg-card border border-border shadow-xs flex flex-col items-center justify-between p-2">
                  ...
                  <div className="flex gap-1 justify-center">
                    <span className="size-2 rounded-full shadow-xs" style={{ background: settings.prepleatDotColor ?? "#ffa029" }} title="PrePleat" />
                    <span className="size-2 rounded-full shadow-xs" style={{ background: settings.directDrapeDotColor ?? "#10b981" }} title="Direct Drape" />
                    <span className="size-2 rounded-full shadow-xs" style={{ background: settings.artistDotColor ?? "#d4af37" }} title="Artist" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5">
              {([
                { key: "prepleatDotColor" as const, label: "PrePleat Dot", def: "#ffa029", desc: "PrePleat bookings color" },
                { key: "directDrapeDotColor" as const, label: "Direct Drape Dot", def: "#10b981", desc: "Direct drape bookings color" },
                { key: "artistDotColor" as const, label: "Artist Dot", def: "#d4af37", desc: "Artist bookings color" },
              ]).map((item) => {
                const val = settings[item.key] || item.def;
                return (
                  <div key={item.key} className="flex items-center gap-3 p-3 rounded-xl bg-card border border-border hover:border-primary/50 transition">
                    <div className="relative size-9 rounded-full border border-border/85 overflow-hidden shadow-xs cursor-pointer">
                      <input
                        ...
                        className="absolute inset-0 size-full border-0 cursor-pointer p-0 opacity-0 z-10"
                      />
                      <div className="absolute inset-0 rounded-full border" style={{ backgroundColor: val }} />
                    </div>
                    <div className="min-w-0">
                      <span className="block text-xs font-bold text-foreground leading-tight">{item.label}</span>
                      <span className="block text-[9px] text-muted-foreground font-mono uppercase mt-0.5">{val}</span>
                      <span className="block text-[9px] text-muted-foreground/80 mt-0.5 leading-none">{item.desc}</span>
                    </div>
                  </div>
                );
              })}
      ...
      {tab === "data" && (
        <>
          ...
          <Accordion type="single" collapsible className="w-full mb-3">
            <AccordionItem value="activity-log" className="border border-border bg-card rounded-2xl px-4 py-1 mb-2.5 card-shadow">
              <AccordionTrigger className="hover:no-underline py-3.5 cursor-pointer">
                <div className="flex flex-col text-left">
                  ...
                  <span className="text-[11px] text-muted-foreground font-medium mt-0.5">Undo last edit or revert specific changes</span>
                </div>
              </AccordionTrigger>
            ...
            <AccordionItem value="recently-deleted" className="border border-border bg-card rounded-2xl px-4 py-1 card-shadow">
              <AccordionTrigger className="hover:no-underline py-3.5 cursor-pointer">
                <div className="flex flex-col text-left">
                  <span className="text-sm font-semibold flex items-center gap-2 text-foreground">
                    <Trash2 className="size-4 text-primary" /> Recently Deleted Bin ({trash.length})
                  </span>
                  <span className="text-[11px] text-muted-foreground font-medium mt-0.5">Restore bookings deleted in the last 7 days</span>
                </div>
              </AccordionTrigger>
              <AccordionContent className="pt-2 pb-4">
                {trash.length === 0 ? (
                  <p className="text-xs text-muted-foreground py-2 text-center">No deleted bookings in recycle bin.</p>
                ) : (
                  <ul className="space-y-2 mt-2">
                    {trash.map((t) => {
                      const c = customers.find((x) => x.id === t.booking.customerId);
                      return (
                        <li key={t.booking.id} className="flex items-center justify-between gap-2 p-2.5 rounded-xl bg-secondary/50">
                          <div className="min-w-0">
                            <p className="text-xs font-semibold truncate">{c?.name ?? "Unknown"} · {t.booking.service}</p>
          ...
          <Section title="Reset & Danger Zone">
            {dataLocked ? (
              <div className="flex flex-col items-center justify-center py-6 bg-destructive/5 border border-dashed border-destructive/20 rounded-2xl text-center">
                ...
                <p className="text-[10px] text-muted-foreground max-w-xs mb-3 px-4">Contains sensitive actions that can reset pricing, styles, or permanently delete data.</p>
                <button
                  onClick={() => {
                    setDataLocked(false);
