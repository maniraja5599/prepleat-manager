...
export const Route = createFileRoute("/book")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Request a PrePleat or Drape booking. We'll confirm by WhatsApp.",
      },
    ],
  }),
      ...
      <div className="min-h-[100dvh] bg-background flex items-center justify-center px-5">
        <div className="max-w-sm w-full bg-card card-shadow rounded-3xl p-8 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-2">
            This booking link is missing a recipient. Please use the personal booking link shared
            with you.
          </p>
        </div>
      </div>
    ...
    return (
      <div className="min-h-[100dvh] bg-background flex items-center justify-center px-5">
        <div className="max-w-sm w-full bg-card card-shadow rounded-3xl p-8 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-2">
            We'll confirm your booking on WhatsApp at{" "}
            ...
            <div className="grid grid-cols-2 gap-2">
              {(["prepleat", "drape"] as const).map((s) => (
                <button
                  ...
                  className={`py-2.5 rounded-full text-sm font-semibold uppercase tracking-wider ${service === s ? "saree-gradient text-primary-foreground" : "bg-secondary"}`}
                >
                  {s}
                </button>
              ))}
          ...
          <Field label="Saree count">
            <div className="flex items-center gap-3">
              <button
                ...
                className="size-9 rounded-full bg-secondary font-bold"
              >
                −
              </button>
